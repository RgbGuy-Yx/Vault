...
type RoomState = "checking" | "active" | "expired" | "destroyed" | "disconnected" | "invalid";
...
export default function RoomPage() {
  ...
  const [expiresAt, setExpiresAt] = useState<number>(() => Date.now() + ROOM_TTL_MS);
  const [remainingMs, setRemainingMs] = useState(ROOM_TTL_MS);
  const [showDestroyConfirm, setShowDestroyConfirm] = useState(false);
  const [statusText, setStatusText] = useState(
    isInvalidRoom ? "Invalid room link" : "Checking room...",
  );
  ...
  const progress = Math.max(0, Math.min(100, (remainingMs / ROOM_TTL_MS) * 100));
  const inputDisabled = roomState !== "active";

  const addSystemMessage = useCallback((body: string) => {
    setMessages((prev) => [
      ...prev,
      {
  ...
  useEffect(() => {
    activeRef.current = roomState === "active";
    terminalRef.current =
  ...
  const enterTerminalState = useCallback(
    (state: Extract<RoomState, "expired" | "destroyed" | "invalid">) => {
  ...
  useEffect(() => {
    ...
    async function verifyRoom() {
      try {
        ...
        if (!res.ok) {
          enterTerminalState(res.status === 410 ? "destroyed" : "expired");
          return;
        }
  ...
  }, [enterTerminalState, isInvalidRoom, roomId]);
  ...
  }, [enterTerminalState, expiresAt, roomState]);

  const scheduleReconnect = useCallback(() => {
    if (terminalRef.current || reconnectTimerRef.current !== null) {
      return;
    }
    ...
    setRoomState("disconnected");
    setStatusText(`Reconnecting ${attempt}/${MAX_RECONNECT_ATTEMPTS}`);
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(`${protocol}://${window.location.host}/api/ws?roomId=${roomId}`);
    ...
    socket.addEventListener("open", () => {
      ...
      if (!joinedSystemMessageRef.current) {
        ...
        addSystemMessage("You joined as an anonymous guest.");
      }
    });
    ...
    socket.addEventListener("message", (event) => {
      ...
      const mine = payload.name === guestName;
      setMessages((prev) =>
        [
          ...prev,
          {
  ...
  }, [addSystemMessage, enterTerminalState, guestName, roomId, roomState, scheduleReconnect]);
  ...
  const sendMessage = (e: FormEvent) => {
    ...
    if (!text || inputDisabled || terminalRef.current || socket?.readyState !== WebSocket.OPEN) {
      return;
    }

    socket.send(JSON.stringify({ type: "MESSAGE", text, name: guestName }));
    setDraft("");
    setStickToBottom(true);
  };
  ...
  const destroyRoom = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Failed to destroy room");
      enterTerminalState("destroyed");
    } catch {
      setRoomState("disconnected");
      setStatusText("Disconnected");
      ...
      <div className="flex min-h-screen flex-col">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-[#3b1111] bg-black px-4 md:px-6">
          <div className="flex h-full items-center gap-8">
            <p className="font-[var(--font-space-grotesk)] text-xl font-bold uppercase text-[#ff3434] drop-shadow-[0_0_12px_rgba(255,52,52,0.55)]">
              Signal_Terminal
            </p>
        ...
        <div className="grid min-h-0 flex-1 lg:grid-cols-[220px_minmax(0,1fr)_300px]">
          <aside className="hidden border-r border-[#3b1111] bg-[#050505] lg:flex lg:flex-col">
            <div className="border-b border-[#3b1111] p-6">
              <p className="font-[var(--font-space-grotesk)] text-xl font-bold uppercase text-[#ff3434]">
                Anonymous
              </p>
          ...
          <section className="flex min-h-0 flex-col border-r border-[#3b1111] bg-[#030303]">
            <div className="flex shrink-0 flex-col border-b border-[#3b1111] bg-[#090606] md:flex-row md:items-center md:justify-between">
              <div className="flex min-w-0 items-center gap-4 px-4 py-3 font-mono text-xs uppercase tracking-[0.18em] md:px-6">
                ...
                <span className="truncate text-zinc-600">Room_{roomId.toUpperCase()}</span>
              </div>
            ...
            <div className="grid shrink-0 gap-3 border-b border-[#3b1111] bg-black/40 p-4 md:hidden">
              <div className="flex items-center justify-between">
                ...
                <span className="font-[var(--font-space-grotesk)] text-3xl text-[#ff3434]">
            ...
            <div className="relative flex min-h-0 flex-1 flex-col">
              <div className="pointer-events-none absolute inset-0 opacity-45 [background-image:linear-gradient(rgba(255,52,52,0.08)_1px,transparent_1px),linear-gradient(90deg,rgba(255,52,52,0.08)_1px,transparent_1px)] [background-size:24px_24px]" />
          ...
          {roomState === "expired" || roomState === "destroyed" || roomState === "invalid" ? (
            ...
            <>
                ...
                {messages.length === 0 ? (
                  <div className="flex min-h-full items-center justify-center text-center">
                    <div className="max-w-md border border-[#3b1111] bg-black/70 p-8">
                      ...
                      <p className="mt-3 font-[var(--font-space-grotesk)] text-3xl uppercase text-zinc-100">
                        No messages yet.
                      </p>
                    ...
                    {messages.map((message) =>
                      message.system ? (
                        ...
                        <div key={message.id} className={message.mine ? "flex justify-end" : "flex justify-start"}>
                          ...
                          >
                            <div
                              className={`mb-2 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.12em] text-zinc-600 ${
                            ...
                            <div
                              className={`border bg-[#121212]/95 px-5 py-4 text-base leading-7 shadow-[0_0_28px_rgba(0,0,0,0.45)] md:text-lg ${
                                message.mine
                            ...
                            <div
                              className={`mt-2 flex gap-2 font-mono text-[9px] uppercase tracking-[0.12em] text-[#ff3434] ${
                                message.mine ? "justify-end" : "justify-start"
                              }`}
                            >
              ...
              <form onSubmit={sendMessage} className="shrink-0 border-t border-[#3b1111] bg-black p-4">
          ...
          <aside className="hidden bg-[#0b0b0b] lg:flex lg:flex-col">
              ...
              <section>
                ...
                <div className="mt-5 space-y-5">
                  <div>
                    ...
                    <p className="mt-1 font-[var(--font-space-grotesk)] text-xl text-zinc-100">
                      {roomId.toUpperCase()}
                    </p>
                  </div>
                  <div>
                    ...
                    <p
                      className={`mt-1 font-[var(--font-space-grotesk)] text-5xl ${
                        dangerLevel === "normal" ? "text-[#ff3434]" : "text-[#ff3434]"
                      }`}
                    >
                      {formatClock(remainingMs)}
                    </p>
                    <div className="mt-4 h-1 bg-[#221010]">
                      <div
                        className={`h-full transition-all duration-300 ${
                          dangerLevel === "warning" ? "bg-amber-300" : "bg-[#ff3434]"
                        }`}
            ...
            <div className="mt-auto space-y-5 p-6">
              ...
              <button
                onClick={() => setShowDestroyConfirm(true)}
                disabled={roomState !== "active"}
                className="h-16 w-full border-2 border-[#ff3434] font-[var(--font-space-grotesk)] text-lg font-bold uppercase tracking-[0.2em] text-[#ff3434] shadow-[0_0_28px_rgba(255,52,52,0.18)] hover:bg-[#ff3434] hover:text-black disabled:cursor-not-allowed disabled:opacity-40"
              >
                Destroy_Session
              </button>
            </div>
      ...
      {showDestroyConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 px-4">
          <div className="w-full max-w-md border border-[#ff3434] bg-[#090909] p-6 shadow-[0_0_50px_rgba(255,52,52,0.2)]">
            ...
            <h2 className="mt-3 font-[var(--font-space-grotesk)] text-3xl uppercase">
              Destroy this room?
            </h2>
  ...
  return (
    <div className="flex flex-1 items-center justify-center p-6 text-center">
      <div className="max-w-md">
        ...
        <h2 className="mt-3 font-[var(--font-space-grotesk)] text-4xl uppercase">{copy.title}</h2>
